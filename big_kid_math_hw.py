...
def form_subtraction():
    ...
    a, b = rng.choice(99, size=2, replace=True, p=None)
    if a >= b:
        top = a
        bottom = b
    else:
...
def numbers_and_factors():
    rng = np.random.default_rng()
    num = rng.choice(max_number*5, size=1, replace=True, p=None)[0]
...
def form_factorize():
    rng = np.random.default_rng()
    num = rng.choice(50, size=1, replace=False, p=None)[0]
    return f'\\fact{{{num}}}' 
...
problems = get_rows_of_random_problems(12) 
